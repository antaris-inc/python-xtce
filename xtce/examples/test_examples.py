...
class TestCCSDS_660x1g2(unittest.TestCase):
    ...
    def test_PWHTMR(self):
        ...
        cmd = xtcemsg.Message(
            ...
            entries={
                'ID': 0x10, #NOTE(bcwaldon): this should be 0x100?
                'SecH': 0,
                'Type': 0,
                'Length': 0,
                #'TimerStartStop': 'TIMER_START', #NOTE(bcwaldon): should allow using labels somehow
                'TimerStartStop': 1,
                'CheckSum': 12,
            },
        )
...
class TestCCSDS_660x2g2(unittest.TestCase):
    ...
    def test_PM1Enable_Logging(self):
        ss = xtceschema.from_file(self.loc)
        self.assertEqual(ss.name, 'GovSat')
